...
def inject_globals():
    """Signed-in user (or None) and current year for every template (header/footer)."""
    current_user = None
    if 'user_id' in flask_session:
        session = Session()
        current_user = session.query(User).filter_by(id=flask_session['user_id']).first()
        session.close()
    return {"current_user": current_user, "current_year": datetime.date.today().year}


@app.route("/")
def index():
    session = Session()
    recent = session.query(Products).order_by(Products.id.desc()).limit(12).all()
    product_count = session.query(Products).count()
    member_count = session.query(User).count()
    stock_total = session.query(func.sum(Products.amount)).scalar() or 0
    session.close()
    return render_template("index.html", featured=recent[:4], ticker=recent,
                           product_count=product_count, member_count=member_count,
...
def require_admin():
    """Return the signed-in admin. Everyone else (guests and normal users) gets a 404,
    so the admin area is invisible to them — it doesn't even reveal that it exists."""
...
def login():
    if request.method == "POST":
        ...
        if user:
            ...
            return render_template("auth.html", mode="login", error="Wrong Name or Password")
...
def reg():
    if request.method == "POST":
        ...
        if existing:
            session.close()
            return render_template("auth.html", mode="register", error="This name already exist.")
